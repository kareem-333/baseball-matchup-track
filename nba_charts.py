"""
NBA Defensive Analytics — Plotly Chart Builders
Dragon Index & Fortress Rating visualizations.

Column conventions (matching nba_pipeline.py):
  User-facing: DRAGON_INDEX, FORTRESS_RATING, COMBINED_SCORE, POSITION, TEAM_COLOR, …
  Components:  d_rim_n, d_defl_n, d_charges_n, d_steals_n, d_contested_n
...
from nba_pipeline import nba_headshot_url, team_color, get_play_sequence_stats
...
_headshot_cache: dict[int, str] = {}   # player_id → base64 data-URI


def _circular_headshot_b64(player_id: int, border_hex: str = "#888888", size: int = 80) -> str | None:
    """Fetch headshot from CDN, clip to circle with a team-colour ring, return base64 data URI."""
...
def plot_bubble_scatter(
    df: pd.DataFrame,
    position_filter: str | None = None,   # None or "Guard"/"Wing"/"Big"
    ...
    Four quadrant labels. Optional position filter.
    """
    ...
    for x0, x1, y0, y1, label, color in quads:
        ...
        fig.add_annotation(
            ...
            text=f"<b>{label}</b>",
            showarrow=False,
            font=dict(size=11, color="rgba(220,220,220,0.30)"),
        )
    ...
    # ── Player bubbles — single consolidated trace per group ─────────────────
    # Using one trace per player causes 500+ traces and breaks %{name} hover.
    # Group by highlight/top10 status to keep it to ≤3 traces total.
    ...
    for gname, gdf in groups.items():
        ...
        lwidths = [style["line_width"]] * len(gdf)
        lcolors = (
            ["white"] * len(gdf) if style["line_color"] == "white"
            else colors
        )

        fig.add_trace(go.Scatter(
            x=gdf["DRAGON_INDEX"].tolist(),
            y=gdf["FORTRESS_RATING"].tolist(),
            mode="markers",
            marker=dict(
                size=sizes,
                color=colors,
            ...
            # customdata cols: [0]=name, [1]=team, [2]=dragon, [3]=fortress,
            #                  [4]=min, [5]=pos, [6]=player_id
            hovertemplate=(
    ...
    # ── Top-10 headshots or name labels ──────────────────────────────────────
    ...
    for _, row in top10.iterrows():
        ...
        if img_b64:
            fig.add_layout_image(dict(
                ...
                xref="x", yref="y",
                x=row["DRAGON_INDEX"],
                y=row["FORTRESS_RATING"],
                sizex=max(x_range * 0.07, 5),
                sizey=max(y_range * 0.11, 7),
            ...
            fig.add_annotation(
                x=row["DRAGON_INDEX"],
                y=row["FORTRESS_RATING"] + y_range * 0.03,
...
def plot_leaderboard(
    df: pd.DataFrame,
    metric: str = "DRAGON_INDEX",   # "DRAGON_INDEX" or "FORTRESS_RATING"
    n: int = 20,
) -> go.Figure:
    """
    Horizontal stacked bar leaderboard (top-N). Each segment = one normalised component.
    Accepts metric as "DRAGON_INDEX"/"DRAGON" or "FORTRESS_RATING"/"FORTRESS".
    """
    if df.empty:
    ...
    if score_col not in df.columns:
        return go.Figure(layout=go.Layout(
            title=f"{score_col} not found in data", **_layout()))

    avail_comps = [c for c in comp_cols if c in df.columns]

    keep_cols = ["PLAYER_ID", "PLAYER_NAME", "TEAM_ABBREVIATION", "TEAM_COLOR", score_col]
    keep_cols += avail_comps
    ...
    for col, label, color in zip(avail_comps, comp_labels, colors):
        vals = top[col] * 20   # 0-1 → 0-20 visual width
    ...
    for i, (_, row) in enumerate(top.iterrows()):
        ...
        fig.add_annotation(
            x=seg_total + 0.5, y=y_labels[i],
            text=f"<b>{row[score_col]:.0f}</b>",
            showarrow=False,
            font=dict(size=9, color="white"),
        )

    title = ("Dragon Index — Top 20 Disruptors"
             if is_dragon else "Fortress Rating — Top 20 Interior Anchors")
    fig.update_layout(
        barmode="stack",
        title=title,
...
def plot_comparison_radar(players_df: pd.DataFrame) -> go.Figure:
    ...
    categories = _RADAR_CATS + [_RADAR_CATS[0]]   # close the polygon
    fig = go.Figure()
...
def plot_comparison_rolling(rolling_data: dict[str, pd.DataFrame], metric: str) -> go.Figure:
    ...
    `metric` accepts "DRAGON_INDEX"/"DRAGON" or "FORTRESS_RATING"/"FORTRESS".
    """
    fig = go.Figure()
    fallback_colors = ["#EB6E1F", "#3498db", "#2ecc71"]
    is_dragon = metric.upper() in ("DRAGON_INDEX", "DRAGON")
...
def plot_team_bubbles(team_df: pd.DataFrame) -> go.Figure:
    """
    Team aggregate bubble: DRAGON_INDEX avg (X) vs FORTRESS_RATING avg (Y).
    Bubble size ∝ AVG_MIN. Colour = team primary colour.
    """
...
def plot_steal_chain_sankey(seq_stats: dict, player_name: str) -> go.Figure:
    """
    Sankey diagram: Steals → scored/not scored → fast break / half-court.

    seq_stats is the dict returned by get_play_sequence_stats().
    """
    if not seq_stats or seq_stats.get("total_steals", 0) == 0:
        return go.Figure(layout=go.Layout(
            title=f"{player_name} — No steal data available",
            **_layout()))
    ...
    # Nodes: 0=Steals, 1=Scored, 2=No Score, 3=Fast Break, 4=Half-Court
    labels = [
        f"Steals<br>({n})",
        f"Scored<br>({scored})",
        f"No Score<br>({no_score})",
        f"Fast Break<br>({fast_break})",
        f"Half-Court<br>({half_court})",
    ]
    ...
    fig.update_layout(
        title=f"{player_name} — Steal → Points Chain  "
              f"(last {seq_stats['games_analyzed']} games)",
        height=380,
...
def _hex_rgba(hex_color: str, alpha: float) -> str:
    """Convert '#RRGGBB' to a valid 'rgba(r,g,b,a)' string."""
    h = hex_color.lstrip("#")
...
def plot_sequence_comparison(stats_dict: dict[str, dict], colors: list[str] | None = None) -> go.Figure:
    ...
    stats_dict: {player_name: seq_stats_dict, ...}
    """
    if not stats_dict:
    ...
    # Build per-player solid and semi-transparent colours
    ...
    fig.add_trace(go.Bar(
        name="Conversion % (steal → any score)",
        x=names, y=conv_pcts,
        marker_color=solid,
        text=[f"{v:.0f}%" for v in conv_pcts],
        textposition="outside",
    ))
    fig.add_trace(go.Bar(
        name="Fast Break % (score within 2 plays)",
        x=names, y=fb_pcts,
        marker_color=faded,
        marker_pattern_shape="/",
        text=[f"{v:.0f}%" for v in fb_pcts],
        textposition="outside",
    ))

    # Secondary axis: pts per steal
    fig.add_trace(go.Scatter(
        name="Pts per Steal",
        x=names, y=pts_per_stl,
        mode="markers+text",
        marker=dict(size=14, color="white", symbol="diamond",
                    line=dict(color="#EB6E1F", width=2)),
        text=[f"{v:.2f}" for v in pts_per_stl],
