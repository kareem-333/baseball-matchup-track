"""
dashboard/components/season_charts.py — Season-level Plotly chart builders.
...
def plot_krate_chart(
    ...
    pitcher_name: str,
    n: int = 15,
    league_avg_kpct: float | None = None,
    league_avg_k9: float | None = None,
) -> go.Figure:
    fig = go.Figure()
...
def plot_career_splits_table_fig(career_df: pd.DataFrame, batter_name: str) -> go.Figure:
    """Colour-coded table of career pitch-type stats, percentile-coloured per column."""
    if career_df.empty:
